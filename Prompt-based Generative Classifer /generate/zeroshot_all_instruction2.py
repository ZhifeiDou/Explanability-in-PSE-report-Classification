...
if COUNTER:
    ...
    OUTPUT_CSV = f"output/output_zeroshot_instruction2_{MODEL}_p{TOPP}_t{TEMP}_seed{SEED}_counter.csv"

else:
    TEST_CSV_PATH = f"PSE_test_data.csv"
    #OUTPUT_CSV = f"output/output_zeroshot_instruction2_{MODEL}_p{TOPP}_t{TEMP}_seed{SEED}.csv"
    OUTPUT_CSV = "test.csv"
...
#--------load pipeline for LLama------------------
#todo: follow up this during summer, Llama4 is out so try llama 4, but MeLlama also doesn't work
#figure out the template, it should work
if MODEL in ['Llama8B', 'MeLlama']:
...
for _, row in tqdm(df_infer.iterrows(), total=len(df_infer), desc="Inferencing"):
    ...
    if MODEL in ["QwQ", "Qwen32B", "Qwen14B"]:
        #accommodate for template of QwQ
        print(f"The template has been activated for {MODEL}")
    ...
    elif MODEL in ['Llama8B', 'MeLlama']:
        #llama models has issues with apply chat template in recent update use pipeline instead
        messages = [
        ...
        print('The output text for Llama is:', output_text)
    ...
    #catch the explanation and prediction label from the output
    #there should be two pairs match the pattern, obtain last one
